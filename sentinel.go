...
func NewSentinel(primaryName string, sentinelAddrs []string, opts ...SentinelOpt) (*Sentinel, error) {
	...
	{
		pconn := PersistentPubSub("", "", func(_, _ string) (Conn, error) {
			return sc.dialSentinel()
		})
		if pconn == nil {
			return nil, errors.New("sential pubsub connection error")
		}
		sc.pconn = pconn

		if err := sc.pconn.Subscribe(sc.pconnCh, "switch-master"); err != nil {
			return nil, err
		}
	}
