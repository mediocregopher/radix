...
func (scl *clusterStub) clientInitPartFailedFunc(failedAddrsMap map[string]bool) ClientFunc {
	return func(network, addr string) (Client, error) {
		for _, s := range scl.stubs {
			if s.addr == addr {
...
func (scl *clusterStub) newInitErrorCluster(failedAddrMap map[string]bool, opts ...ClusterOpt) (*Cluster, error) {
	opts = append([]ClusterOpt{ClusterPoolFunc(scl.clientInitPartFailedFunc(failedAddrMap))}, opts...)
	return NewCluster(scl.addrs(), opts...)
}
