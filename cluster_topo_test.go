...
func (a respArr) MarshalRESP(w io.Writer, o *resp.Opts) error {
	if err := (resp3.ArrayHeader{NumElems: len(a)}).MarshalRESP(w, o); err != nil {
		return err
	}
	for _, el := range a {
		if err := resp3.Marshal(w, el, o); err != nil {
...
var testTopo = func() ClusterTopo {
	...
	opts := resp.NewOpts()
	if err := testTopoResp.MarshalRESP(buf, opts); err != nil {
		panic(err)
	}
	var tt ClusterTopo
	if err := tt.UnmarshalRESP(bufio.NewReader(buf), opts); err != nil {
		panic(err)
	}
	return tt
}()
...
func TestClusterTopo(t *T) {
	...
	opts := resp.NewOpts()
	buf := new(bytes.Buffer)
	require.Nil(t, testTopo.MarshalRESP(buf, opts))
	var testTopo2 ClusterTopo
	require.Nil(t, testTopo2.UnmarshalRESP(bufio.NewReader(buf), opts))
